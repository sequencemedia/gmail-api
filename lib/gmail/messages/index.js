 ...
 * @param {number} n - Next iteration (i + 1)
 * @return {Function} Returns function
 */
const mapMessagesListPage = (parameters = {}, messagesList = [], i = 0, n = i + 1) => {
  /**
   * @param {object} Gmail `message.list()` response
   * @return {Promise<array>|array} Returns a promise resolving to an array, or an array
   */
  return async ({ data: { nextPageToken: token, messages: page = [] } = {} } = {}) => {
    /**
