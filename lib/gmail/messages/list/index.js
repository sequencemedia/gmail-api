 ...
 * @returns {Promise<array>} Returns a promise resolving to an array
 */
async function getMessages (gmail, { user = 'me', query = '', max = Number.POSITIVE_INFINITY } = {}) {
  const map = mapMessagesList({ gmail, user, query, max })

  return map(await getMessagesList({ gmail, user, query, max, limit: getLimit(max) }))
}
