 ...
 */
const getDraftsListPage = async ({ gmail: { users: { drafts } }, ...parameters } = {}) => drafts.list(transformToDraftsListParameters(parameters))
 ...
 */
function mapDraftsListPage (parameters = {}, draftsList = [], i = 0, n = i + 1) {
  /**
  ...
  return async function map ({ data: { nextPageToken: token = '', drafts = [] } = {} } = {}) {
    ...
    if (token) {
       ...
       */
      if (limit) { // 1 ~ 500 || 0
        const map = mapDraftsListPage(parameters, draftsList.concat(drafts), n)

        /**
         *  Fetch another page (1 ~ 500)
         */
        return map(await getDraftsListPage({ ...parameters, token, limit }))
      }
    }
 ...
 * @returns {Promise<array>} Returns a promise resolving to an array
 */
async function getDraftsList (gmail, { user = 'me', query = '', max = Number.POSITIVE_INFINITY } = {}) {
  const map = mapDraftsListPage({ gmail, user, query, max })

  return map(await getDraftsListPage({ gmail, user, query, max, limit: getLimit(max) }))
}
