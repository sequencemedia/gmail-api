 ...
 * @returns {Promise<array>} Returns a promise resolving to an array
 */
async function getDrafts (gmail, { user = 'me', query = '', max = Number.POSITIVE_INFINITY } = {}) {
  const map = mapDraftsList({ gmail, user, query, max })

  return map(await getDraftsList({ gmail, user, query, max, limit: getLimit(max) }))
}
