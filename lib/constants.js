...
const getCredentialsPathEnvValue = ({ env = {} } = process) => Reflect.has(env, 'CREDENTIALS_PATH') ? Reflect.get(env, 'CREDENTIALS_PATH') : 'credentials.json'

const getCredentialsTokenPathEnvValue = ({ env = {} } = process) => Reflect.has(env, 'CREDENTIALS_TOKEN_PATH') ? Reflect.get(env, 'CREDENTIALS_TOKEN_PATH') : 'credentials-token.json'
